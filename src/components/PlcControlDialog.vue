...
interface SwitchItem {
  /** 下发参数名 — 对齐 Qt setPLC params；只读行仅作 key */
  key: string
  label: string
  /** 本地状态字段 */
  field: string
  /** 确认文案：next=true / next=false（只读行可空） */
  tipOn?: string
  tipOff?: string
  /** STATUS 反馈：仅显示，不可点控 */
  readonly?: boolean
...
/** 可下发控制项 — 对齐原 PLCControlDialog.ui */
const controlSwitches: SwitchItem[] = [
